...
class SignalHandler:
    ...
    def _get_crisis_impact(self, symbol: str, crisis_report: Dict) -> Dict:
        ...
        return {
            'impact_level': 'MEDIUM',
            'scenarios_affected': ['TECH_CRASH', 'LIQUIDITY_SHOCK'],
            'recommendation': 'Monitor closely'
        }
    
    def _generate_recommendation(self, instrument_data: Dict, risk_assessment: Dict) -> Dict:
        """تولید توصیه معاملاتی"""
        price_change = instrument_data.get('change_percent', 0)
        volatility = risk_assessment.get('volatility', 0)
        
        if price_change > 5 and volatility < 0.1:
            return {
                'action': 'BUY',
                'confidence': 'HIGH',
                'reason': 'Strong upward trend with low volatility'
            }
        elif price_change < -5 and volatility > 0.15:
            return {
                'action': 'SELL', 
                'confidence': 'MEDIUM',
...
class PortfolioHandler:
    """Handler برای مدیریت پرتفولیو"""
    
    def __init__(self):
        self.logger = logging.getLogger(__name__)
    
    async def analyze_portfolio(self, portfolio_data: Dict, user_id: str) -> Dict:
        """آنالیز پرتفولیو"""
        try:
            self.logger.info(f"📊 Analyzing portfolio for user {user_id}")
            
            # اعتبارسنجی داده‌های پرتفولیو
            is_valid, errors = data_validator.validate_market_data(portfolio_data)
            if not is_valid:
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST,
                    detail=f"Invalid portfolio data: {errors}"
                )
            
            # تحلیل ریسک با مونت کارلو
            risk_analysis = await monte_carlo_engine.analyze_portfolio_risk(portfolio_data)
            
            # تحلیل بحران
            crisis_analysis = await crisis_analyzer.analyze(portfolio_data)
            
            return {
                'status': 'success',
                'risk_metrics': risk_analysis.get('risk_metrics', {}),
                'crisis_impact': crisis_analysis.get('crisis_score', 0),
                'recommendations': self._generate_portfolio_recommendations(risk_analysis, crisis_analysis),
                'optimization_suggestions': await self._generate_optimization_suggestions(portfolio_data)
            }
    ...
    async def get_optimal_allocation(self, user_preferences: Dict, user_id: str) -> Dict:
        """پیدا کردن تخصیص بهینه دارایی‌ها"""
        try:
            self.logger.info(f"🎯 Calculating optimal allocation for user {user_id}")
            ...
            # محاسبه تخصیص بر اساس ترجیحات کاربر
            allocation = self._calculate_optimal_allocation(risk_tolerance, investment_horizon)
            ...
            return {
                ...
                'risk_estimate': self._calculate_risk_estimate(allocation)
            }
            ...
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                detail="Failed to calculate optimal allocation"
            )
    
    def _generate_portfolio_recommendations(self, risk_analysis: Dict, crisis_analysis: Dict) -> List[Dict]:
        """تولید توصیه‌های پرتفولیو"""
        recommendations = []
        
        var_95 = risk_analysis.get('risk_metrics', {}).get('var_95', 0)
        ...
        if var_95 < -0.1:
            recommendations.append({
                ...
                'details': 'Consider increasing cash position or adding hedging instruments'
            })
        
        if crisis_score > 0.6:
            recommendations.append({
                'type': 'CRISIS_PREPARATION',
                'priority': 'HIGH', 
                'action': 'Prepare for market crisis',
                'details': 'Increase allocation to safe-haven assets like gold and bonds'
            })
        ...
        if len(recommendations) == 0:
            recommendations.append({
                ...
                'details': 'Portfolio appears well-balanced for current market conditions'
            })
        
        return recommendations
    
    async def _generate_optimization_suggestions(self, portfolio_data: Dict) -> List[Dict]:
        """تولید پیشنهادهای بهینه‌سازی"""
        # اینجا می‌توان الگوریتم‌های بهینه‌سازی پیچیده‌تری پیاده‌سازی کرد
        return [
            {
                'suggestion': 'Diversify across more asset classes',
                'potential_improvement': '15% risk reduction',
                'implementation': 'Add commodities and real estate exposure'
            },
            {
                'suggestion': 'Rebalance quarterly',
                'potential_improvement': '5% return enhancement', 
                'implementation': 'Set up automatic rebalancing schedule'
            }
        ]
    
    def _calculate_optimal_allocation(self, risk_tolerance: str, investment_horizon: str) -> Dict:
        """محاسبه تخصیص بهینه دارایی‌ها"""
        # مدل‌های تخصیص بر اساس ترجیحات کاربر
        models = {
            'CONSERVATIVE': {
                'stocks': 0.3,
                'bonds': 0.4,
        ...
        return models.get(risk_tolerance, models['MODERATE'])
    
    def _calculate_expected_return(self, allocation: Dict) -> float:
        """محاسبه بازده مورد انتظار"""
        # بازده‌های تاریخی تقریبی
        historical_returns = {
            'stocks': 0.08,
            'bonds': 0.04,
            'cash': 0.02,
        ...
        for asset, weight in allocation.items():
            expected_return += weight * historical_returns.get(asset, 0)
        
        return expected_return
    
    def _calculate_risk_estimate(self, allocation: Dict) -> float:
        """محاسبه تخمین ریسک"""
        # نوسانات تاریخی تقریبی
        ...
        # محاسبه ساده واریانس پرتفولیو
        portfolio_variance = 0
        for asset, weight in allocation.items():
            portfolio_variance += (weight ** 2) * (historical_volatilities.get(asset, 0) ** 2)
        
        return portfolio_variance ** 0.5  # انحراف معیار

# نمونه‌های سینگلتون برای استفاده در سراسر سیستم
